...
async function createUser (req: Request,res: Response){
	...
	try {
		...
		try{
			usersJSON.users.some((user:TUser)=>{
				if(user.email === newEmail) {
					throw new Error();
				}
			})
		} catch(err) {
			return res.status(500).send("Um usuário já existe com esse email")
		}
...
async function updateUser(req: Request,res: Response){
	...
	const reqNewPassword:string = req.body.password;
	...
	if(usersJSON.users.length !== 0) {
		usersJSON.users.some( (user:TUser, index:number) => {
			if( reqId === user.id ){
				...
				return writeDatabase().then(res => {return res});
